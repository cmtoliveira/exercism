...
def letter_grades(highest):
    ...
    for i in range(1,4):
        threshold_list.append(41+interval*i)
...
def student_ranking(student_scores, student_names):
    ...
    for index, student in enumerate(student_scores):
        ranking_list.append(f"{index+1}. {student_names[index]}: {student_scores[index]}")
    return ranking_list    
...
def perfect_score(student_info):
    ...
    for index, student in enumerate(student_info):
        if student[1] == 100:
            ...
            perfect_score_list = [student[0],student[1]]
