...
int monthly_rate(double hourly_rate, double discount) {
    ...
    // discount.
    int billableDays = 22;
    double beforeDiscount = daily_rate(hourly_rate) * billableDays;
    double afterDiscount = beforeDiscount * (100-discount)/100;
    int rounded = ceil(afterDiscount);
    return rounded;
}
...
int days_in_budget(int budget, double hourly_rate, double discount) {
    ...
    double workDay = apply_discount(daily_rate(hourly_rate), discount);    
    int rounded = floor(budget/workDay);
    return rounded;
